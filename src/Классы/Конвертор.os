...
Процедура MarkdownToHtml(Знач Источник, Знач Куда) Экспорт
	...
	// Вариант с добавлением оглавления в начале справки. Требует обязательного указания
	// заголовка (title). todo понять, нужно это или нет.
	// СтрокаЗапуска = СтрШаблон("pandoc -s --toc -M title=""title"" -f markdown -t html %1", Источник.ПолноеИмя);	
	СтрокаЗапуска = СтрШаблон("pandoc -f markdown -t html %1", Источник.ПолноеИмя);	
	Лог.Отладка(СтрокаЗапуска);

	Результат = ВыполнитьКоманду(СтрокаЗапуска);
	Результат = СтрШаблон(ШаблонHtml(), Результат);
	Результат = СтрЗаменить(Результат, "<table>", "<table width=""100%"" rules=""all"" border=""1"">");
	СохранитьТекстовыйФайл(Результат, ИмяФайлаПриемника);
