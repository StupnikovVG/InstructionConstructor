...
//	ДополнитьСтрокуЗапуска(СтрокаЗапуска);
	Лог.Отладка(СтрокаЗапуска);
...
Процедура УстановитьКодовуюСтраницу()
	...
	Команда.УстановитьПравильныйКодВозврата(0);
	Команда.ПоказыватьВыводНемедленно(Ложь);
	Команда.УстановитьСтрокуЗапуска("chcp 65001");
	Команда.Исполнить();	
КонецПроцедуры
...
Процедура ДополнитьСтрокуЗапуска(СтрокаЗапуска)
	
	// добавление рамок таблиц
	СтрокаЗапуска = СтрокаЗапуска + " | 
	|sed -r '/s/<table>/<table border=""1"">'";
